...
const removeFromPlaylist = async (playlistId, trackId) => {
  ...
  console.log("PlaylistId:", playlistId);
  try {
    const data = await $fetch(`http://localhost:3001/api/playlist/track`, {
      method: "DELETE",
      body: JSON.stringify({
        trackPlaylistT_id: trackId,
        trackPlaylistP_id: playlistId,
    ...
    <table class="table min-h-full">
      ...
      <tbody>
        ...
        <tr
          ...
          v-for="(track, index) in albumTracksWithFavorites"
          :key="index"
        >
          <td>
          ...
          <td>
            <div class="dropdown dropdown-end">
              ...
              >
                <li>
                  <a @click="toogleFavorites(track.trackId)">{{
                    track.favorite
                ...
                <li>
                  <a
                    @click="removeFromPlaylist(track.playlistId, track.trackId)"
